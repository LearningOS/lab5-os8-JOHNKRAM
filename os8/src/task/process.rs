...
impl Banker {
    ...
    pub fn add_thread(&mut self) -> usize {
        ...
        let threads = self.allocation.len();
        self.allocation.push(vec![0; resources]);
        self.need.push(vec![0; resources]);
        threads
    }
    fn add_resource(&mut self, size: usize) -> usize {
        let resources = self.available.len();
        let threads = self.allocation.len();
        self.available.push(size);
        for thread in 0..threads {
            self.allocation[thread].push(0);
            self.need[thread].push(0);
        }
        resources
    }
    ...
    pub fn add_sem(&mut self, size: usize) -> usize {
        self.add_resource(size)
    }
    pub fn check(&self, thread: usize, resource: usize) -> bool {
        let resources = self.available.len();
        let threads = self.allocation.len();
        let mut work = self.available.clone();
        let allocation = self.allocation.clone();
        let mut need = self.need.clone();
        let mut finish = vec![false; threads];
        need[thread][resource] += 1;
        loop {
            if let Some(thread) = (0..threads).find(|thread| {
                !finish[*thread]
                    && (0..resources)
                        .find(|resource| need[*thread][*resource] > work[*resource])
                        .is_none()
            }) {
                finish[thread] = true;
                ...
                return finish.iter().find(|finish| !*finish).is_none()
            }
        }
    }
    pub fn need(&mut self, thread: usize, resource: usize) {
        self.need[thread][resource] += 1;
    }
    pub fn alloc(&mut self, thread: usize, resource: usize) {
        self.available[resource] -= 1;
        self.allocation[thread][resource] += 1;
        self.need[thread][resource] -= 1;
    }
    pub fn dealloc(&mut self, thread: usize, resource: usize) {
        self.available[resource] += 1;
        self.allocation[thread][resource] -= 1;
    }
}
